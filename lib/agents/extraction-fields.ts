...
//
// lookupRecordType is optional: when set, the pipeline uses this field's
// extracted value to search that record type (search_records)
...
// lib/entities/). A field with no lookupRecordType behaves exactly as
...
export const extractionFieldSchema = z.object({
  ...
  lookupRecordType: z.string().trim().min(1).optional(),
});
